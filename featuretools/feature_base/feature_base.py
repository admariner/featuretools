...
class FeatureBase(object):
    def __init__(self, entityset, dataframe_name, base_features, relationship_path, primitive, name=None, names=None):
        ...
        Args:
            dataframe (pd.DataFrame, etc): dataframe this feature is being calculated for
    ...
    def hash(self):
        return hash(self.get_name() + self.dataframe.ww.name)  # why do we do this when self.dataframe_name exists?
    ...
    def column_schema(self):  # typing_info ? typing_schema? schema?
        ...
        while column_schema is None:
            ...
            feature = base_feature  # TODO what is this
    ...
    def __mul__(self, other):
        """Multiply by other"""
        if isinstance(other, FeatureBase):
            if self.column_schema.logical_type == ltypes.Boolean and other.column_schema.logical_type == ltypes.Boolean:
