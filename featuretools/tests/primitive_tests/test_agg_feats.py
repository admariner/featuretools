...
def test_get_depth(es):
    ...
    count_logs = ft.Feature(log_id_feat, parent_dataframe=es['sessions'], primitive=Count)
    sum_count_logs = ft.Feature(count_logs, parent_dataframe=es['customers'], primitive=Sum)
    num_logs_greater_than_5 = sum_count_logs > 5
    count_customers = ft.Feature(customer_id_feat,
                                 parent_dataframe=es[u'régions'],
                                 where=num_logs_greater_than_5,
                                 primitive=Count)
    num_customers_region = ft.Feature(count_customers, dataframe=es["customers"])
...
def test_count_null_and_make_agg_primitive(pd_es):
    ...
    count_null = ft.Feature(pd_es['log']['value'], parent_dataframe=pd_es['sessions'], primitive=Count(count_null=True))
...
def test_check_input_types(es):
    count = ft.Feature(es["sessions"]["id"], parent_dataframe=es["customers"], primitive=Count)
    mean = ft.Feature(count, parent_dataframe=es[u"régions"], primitive=Mean)
    assert mean._check_input_types()

    boolean = count > 3
    mean = ft.Feature(count, parent_dataframe=es[u"régions"], where=boolean, primitive=Mean)
    assert mean._check_input_types()
...
def test_base_of_and_stack_on_heuristic(es, test_primitive):
    child = ft.Feature(es["sessions"]["id"], parent_dataframe=es["customers"], primitive=Count)
...
def test_stack_on_self(es, test_primitive):
    # test stacks on self
    child = ft.Feature(es['log']['value'], parent_dataframe=es[u'régions'], primitive=test_primitive)
    test_primitive.stack_on = []
    child.primitive.base_of = []
    ...
    for agg_prim in agg_primitives:
        ...
        for it in input_types:
            ...
            for t in matching_types:
                instance = ft.Feature(t, parent_dataframe=session, primitive=agg_prim)
...
def test_time_since_last(pd_es):
    f = ft.Feature(pd_es["log"]["datetime"], parent_dataframe=pd_es["customers"], primitive=TimeSinceLast)
...
def test_time_since_first(pd_es):
    f = ft.Feature(pd_es["log"]["datetime"], parent_dataframe=pd_es["customers"], primitive=TimeSinceFirst)
    fm = ft.calculate_feature_matrix([f],
                                     entityset=pd_es,
                                     instance_ids=[0, 1, 2],
                                     cutoff_time=datetime(2015, 6, 8))
...
def test_median(pd_es):
    f = ft.Feature(pd_es["log"]["value_many_nans"], parent_dataframe=pd_es["customers"], primitive=Median)
    fm = ft.calculate_feature_matrix([f],
                                     entityset=pd_es,
                                     instance_ids=[0, 1, 2],
                                     cutoff_time=datetime(2015, 6, 8))
...
def test_agg_same_method_name(es):
    ...
    f_sum = ft.Feature(es["log"]["value"], parent_dataframe=es["customers"], primitive=Sum)
    f_max = ft.Feature(es["log"]["value"], parent_dataframe=es["customers"], primitive=Max)
...
def test_custom_primitive_default_kwargs(es):
    ...
    sum_n_1 = ft.Feature([sum_n_1_base_f], parent_dataframe=es['sessions'], primitive=SumNTimes(n=sum_n_1_n))
    sum_n_2_n = 2
    sum_n_2_base_f = ft.Feature(es['log']['value_2'])
    sum_n_2 = ft.Feature([sum_n_2_base_f], parent_dataframe=es['sessions'], primitive=SumNTimes(n=sum_n_2_n))
...
def test_stacking_multi(pd_es):
    ...
    tc = ft.Feature(pd_es['log']['product_id'], parent_dataframe=pd_es["sessions"], primitive=threecommon)

    stacked = []
    for i in range(3):
        stacked.append(ft.Feature(tc[i], parent_dataframe=pd_es['customers'], primitive=NumUnique))
...
def _assert_agg_feats_equal(f1, f2):
    ...
    assert f1.parent_dataframe.id == f2.parent_dataframe.id
